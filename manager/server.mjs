...
const exec=promisify(execFile);const here=path.dirname(fileURLToPath(import.meta.url));const root=path.resolve(here,"..");const managerPublic=path.join(here,"public");const dataFile=path.join(root,"data","knowledge.json");const uploadDir=path.join(root,"uploads");const port=4178;
...
async function git(args){
  ...
  let lastError;
  for(const executable of ["git",bundledGit]){
    try{return await exec(executable,args,{cwd:root,windowsHide:true,maxBuffer:1024*1024})}
    catch(error){lastError=error;if(error.code!=="ENOENT")throw error}
  }
  throw lastError;
}
